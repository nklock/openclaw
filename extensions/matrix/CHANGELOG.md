...
- Matrix/E2EE: persist the crypto IndexedDB snapshot off the main thread. The 60s persist tick previously ran `JSON.stringify(snapshot)` + `fs.writeFileSync` synchronously on the main event loop, blocking matrix sync, agent work, and inbound message decryption for the full duration; production observed 47–155 second windows as the crypto store grew. The serialize+write step now ships to a long-lived `worker_threads` Worker via `postMessage`; the existing advisory file lock stays on the main thread and wraps the dispatch round-trip so concurrent CLI invocations still see well-defined state. The write is atomic (`tmp → fsync → rename → mode 0o600`), closing a prior failure mode where pod OOM-kill mid-write left a truncated snapshot that silently failed restore on the next boot. `MATRIX_IDB_PERSIST_WORKER=off` falls back to the inline path if needed [AI-assisted]. Thanks @nklock.
